...
\begin{document}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{p{1.2cm} p{2.2cm} p{3.2cm} p{3.8cm} p{3cm} p{1cm} }
            ...
            \begin{minipage}[b]{0.9\linewidth}
                \centering
                \raisebox{-0.5\height}{\includegraphics[width=0.9\linewidth]{Img/图片4.png}}
            \end{minipage} \\
            5 & 表面粗糙度改进 & M8螺栓支承面Ra1.6的粗糙度过低 & 将该面粗糙度修正为Ra3.2，与普通螺栓支承面的通用要求匹配，节约成本。 &
            \begin{minipage}[b]{0.9\linewidth}
                \centering
                \raisebox{-0.5\height}{\includegraphics[width=0.9\linewidth]{Img/图片5.png}}
            \end{minipage} \\
            6 & 结构工艺性改进 & 左侧视图阶梯过渡转角为直角，未标注铸造圆角，易产生铸造缺陷与应力集中 & 在该转角处添加 R3~R5 的铸造圆角，按国标绘制圆角轮廓线 &
            \begin{minipage}[b]{0.9\linewidth}
                \centering
                \raisebox{-0.5\height}{\includegraphics[width=0.9\linewidth]{Img/图片6.png}}
            \end{minipage} \\
    ...
    \textbf{CA6140 车床杠杆加工工艺及夹具设计报告}

    \vspace{0.3cm}

    本报告详细阐述了 CA6140 车床杠杆零件的机械加工工艺规程设计。该零件主要用于支撑与固定，其加工质量直接影响机床的装配精度与运行稳定性。本设计遵循“先面后孔”的原则，将加工过程划分为粗加工和精加工阶段，并合理选择基准以确保加工精度。

    \subsection{机械加工工艺分析}

    \subsubsection{\textbf{零件加工的毛坯选择分析}}

    零件CA6140车床杠杆的材料为 HT200，而且零件的轮廓尺寸不大，铸造表面质量的要求高，故可采用铸造质量稳定的，适合大批生产的金属模铸造。考虑到本零件在具体工作的受力情况，也可以选择砂型铸造。最终选择金属模铸造，其便于铸造和加工工艺过程，而且还可以提高生产率。根据灰口铸铁 HT200 的密度以及给定的零件图计算出毛坯质量约为 1kg，生产类型属于轻型零件，下图为杠杆零件毛坯草图。

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.92\textwidth]{Img/2.1.1.png}
        \caption{杠杆零件工作图}
    \end{figure}

    \subsubsection{\textbf{零件加工的主要加工表面分析}}
    ...
    \begin{table}[H]
        \centering
        \caption{机械工艺路线拟定}
        \begin{tabular}{p{1.5cm} p{6.5cm} p{3cm} p{3cm}}
            \toprule
            工序号 & 工序内容（含主要工步）                                                                                                                                                   & 设备           & 工艺装备/刀具              \\
            \midrule
            10  & \textbf{ 毛坯铸造与处理：}\newline{}粗精铣杠杆下表面。保证粗糙度是3.2选用立式升降台铣床X52K。\newline{}                                                                                        & 铸造及清理设备 & 金属模具、清理工具 \\
            20  & \textbf{划线：}\newline{}划线找正主要轮廓和孔位，划出加工基准线；加工前检验毛坯余量是否合格\newline{}                                                                                             & 划线平台         & 划针、样冲、高度尺            \\
            30  & \textbf{粗、精铣杠杆下表面。}\newline{}以不加工表面为粗基准，保证表面粗糙度达到 3.2。 \newline{}                                                                                             & X52K 立式升降台铣床 & 平口钳或专用夹具、面铣刀 \\
            40  & \textbf{加工 Φ25 中心基准孔。}\newline{}1. 钻孔：Φ25 毛坯孔钻至 Φ23mm； \newline{}2. 扩孔：扩至 Φ24.8mm； \newline{}3. 粗铰：铰至 Φ24.94mm； \newline{}4. 精铰：铰至 Φ25mm，保证粗糙度 1.6。\newline{} & Z535 立式钻床 & 专用夹具、麻花钻、扩孔钻、铰刀 \\
            50  & \textbf{粗、精铣宽度为 30mm 的下平台。}\newline{}以中心孔及端面定位，分次铣削加工台肩。 \newline{}                                                                                           & X52K 立式铣床 & 组合夹具、端铣刀 \\
            60  & \textbf{钻 Φ12.7 锥孔。}\newline{}作为后续加工的辅助定位孔，保证孔的位置度要求。 \newline{}                                                                                              & Z535 立式钻床    & 专用钻模夹具、麻花钻           \\
            70  & \textbf{加工 M8 螺纹孔及锪孔。}\newline{}1. 钻 M8 底孔 Φ6.7； \newline{}2. 锪钻 Φ14 阶梯孔； \newline{}3. 攻 M8 螺纹（保证与垂直方向成 10°）。 \newline{}& Z535 立式钻床 & 组合夹具、麻花钻、锪孔钻、丝锥 \\
            80  & \textbf{粗、精铣 M6 上端面。}\newline{}加工与水平成 36° 的斜面台肩 \newline{}                                                                                                    & X52K 卧式铣床    & 组合夹具、回转分度仪、端铣刀       \\
            90  & \textbf{加工 2-M6 螺纹孔。}\newline{}1. 钻 2-M6 底孔 Φ5.2； \newline{}2. 倒角 120°； \newline{}3. 攻 2-M6 螺纹。 \newline{}& Z535 立式钻床 & 专用夹具、麻花钻、倒角刀、丝锥 \\
            100 & \textbf{钳工精修。}\newline{}精修局部槽面、圆角过渡面，去除各孔口及边缘毛刺。 \newline{}                                                                                                   & 钳工台          & 锉刀、砂布、刮刀             \\
            110 & \textbf{终检。}\newline{}检验中心孔尺寸与粗糙度、孔距、角度、关键平面尺寸（如 Φ12.7 用钢球检验）；防锈入库。\newline{} & 检验平台 & 塞规、钢球、游标卡尺、粗糙度样块、角度尺 \\
            \bottomrule
        \end{tabular}\label{tab:table}
    \end{table}

    \subsubsection{设备选择说明}

    1.\textbf{铣床：}主要用于基准平面、台肩、槽面和斜面的加工。本零件大批量生产中，采用 X52K 立式升降台铣床和卧式铣床即可满足粗、精铣削的要求。

    2.\textbf{钻床：}主要用于孔系加工。中心主孔精度要求较高，一般连接孔和螺纹孔的位置度也有要求，因此本零件的孔系统一采用 Z535 立式钻床，配合相应的刀具（钻、扩、铰、锪）完成集中加工。

    3.\textbf{专用夹具：}考虑到杠杆零件形状不规则，且为大批量生产，在加工 Φ25 中心基准孔、Φ12.7 工艺定位孔及螺纹孔系时，配置专用钻模或组合夹具能显著提高孔位精度和生产率。

    \subsubsection{关键工序卡片式简要描述}

    \begin{table}[H]
        \centering
        \caption{关键工序卡片式简要描述}
        \begin{tabular}{p{1.5cm} p{2.5cm} p{7.5cm} p{2.5cm}}
            \toprule
            工序 & 加工内容             & 定位与夹紧方案                                  & 主要刀具       \\
            \midrule
            30 & 粗、精铣下端面          & 以毛坯外圆弧面及肩台为粗基准，V 型块及支承钉定位，压板夹紧。          & 面铣刀        \\
            40 & 加工 Φ25 主孔（钻、扩、铰） & 以杠杆下表面及 Φ45 孔外圆面为定位基准，在定位块和 V 型块上实现完全定位。 & 麻花钻、扩孔钻、铰刀 \\
            60 & 钻 Φ12.7 锥孔       & 以已加工的 Φ25 中心孔、端面和水平底面为定位基准，长圆柱销、支承板定位。   & 麻花钻        \\
            70 & 加工 M8 螺纹孔及锪阶梯孔 & 以 Φ25 孔及其下表面、宽 30mm 的下平台为定位基准，专用钻模板导向。麻花钻、锪孔钻、丝锥 \\
            \bottomrule
    ...
    \begin{enumerate}
        ...
        \item 王先逵. 《机械制造工艺学》.北京:机械工业出版社.
        \item 艾兴,肖诗纲. 《切削用量简明手册》.北京:机械工业出版社.
        \item 李益民. 《机械制造工艺设计简明手册》.北京:机械工业出版社.
        \item 《机械制造技术基础》课程讲义与课堂资料.
    \end{enumerate}
