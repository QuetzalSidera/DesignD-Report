...
\title{2023年春季学期《机械制造技术基础》课程大作业\\中批量机械加工杠杆零件的关键工艺问题分析}
\author{班号：23级机器人工程4班 \quad 学号：\underline{\hspace{3cm}} \quad 姓名：\underline{\hspace{2.5cm}}}
\date{}
...
\begin{tabular}{p{1.2cm} p{2.2cm} p{4.2cm} p{5.8cm}}
\toprule
序号 & 改进类别 & 错误点 & 改进方案 \\
\midrule
...
对于本杠杆零件而言，按照粗基准的选择原则，选择本零件的不加工表面是加强肋所在的肩台的表面作为加工的粗基准，可用装夹对肩台进行加紧，利用一个V形块支承 45圆的外轮廓作为主要定位，以限制z、z、y、y四个自由度 。再以一面定位消除x、x两个自由度，达到完全定位,就可加工$\Phi25$的孔 。

\subsubsection{精基准选择}

主要考虑到基准重合的问题，和便于装夹，采用$\Phi25$的孔作为精基准，后续工序（如加工 30mm 下平台、$\Phi12.7$ 锥孔及 M8螺纹孔）中，均以此作为定位基准，确保位置精度。
...
\begin{itemize}
  \item 加工 $\Phi25$ 孔： 以 $\Phi25$ 孔下表面及 $\Phi45$ 孔外圆面为定位基准，在定位块和 V 型块上实现完全定位 。
  \item 加工 $\Phi12.7$ 孔： 以 $\Phi25$ 孔、端面和水平底面为定位基准，在长销、支承板和支承钉上实现完全定位 。
  \item 加工 M8 螺纹底孔： 以 $\Phi25$ 孔及其下表面、宽度为 30mm 的下平台为定位基准，在大端面长圆柱销、支承板和支承钉上定位 。
\end{itemize}
